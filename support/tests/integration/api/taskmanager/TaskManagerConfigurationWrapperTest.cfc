		...
		describe( "getConfiguredTasks", function(){
			it( "should discover mix of Tasks.cfc and ScheduledTasks.cfc file in all the passed handler folders and return a structure of tasks that have been derived from the metadata on their handler actions", function(){
				...
				var tasks = {
					...
					, task_2 = { name="Task 2", description="This is scheduled task 2", event="scheduledtasks.task_2", schedule="* 14 3 * * *", timeout=600, priority=0 }
					, task_3 = { name="Task 3", description="This is scheduled task 3", event="scheduledtasks.task_3", schedule="* 5 * * * *" , timeout=600, priority=0 }
					, task_4 = { name="Task 4", description="This is scheduled task 4", event="scheduledtasks.task_4", schedule="* 5 * * * *" , timeout=600, priority=0 }
					, task_5 = { name="Task 5", description="This is scheduled task 5", event="scheduledtasks.task_5", schedule="* 14 3 * * *", timeout=600, priority=0 }
				};
