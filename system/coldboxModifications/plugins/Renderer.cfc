<cfcomponent extends="coldbox.system.Plugin" singleton="true" output="false">
...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<cffunction name="renderView"	access="Public" hint="Renders the current view." output="false" returntype="Any">
		...
		<cfargument name="args"   					required="false" type="any"  default="#getRequestContext().getCurrentViewArgs()#" hint="An optional set of arguments that will be available to this layouts/view rendering ONLY"/>
		...
		<cfargument name="_counter"					required="false" type="any"	 default="0"		hint="Used for preside data object renderers" colddoc:generic="numeric">
		<cfargument name="_records"		 			required="false" type="any"	 default="0"	 	hint="Used for preside data object renderers" colddoc:generic="numeric">
		<!--- ************************************************************* --->
		<cfscript>
			var viewCacheKey 		= "";
			var viewCacheEntry 		= "";
    ...
    <cffunction name="renderViewCollection" output="false" access="public" returntype="any" hint="Render a view composed of collections">
		...
		<cfscript>
			...
			<!--- Query Rendering --->
			<cfset viewArgs._items	= arguments.collection.recordCount>
			<!--- Max Rows --->
			<cfif arguments.collectionMaxRows NEQ 0 AND arguments.collectionMaxRows LTE arguments.collection.recordCount>
				<cfset viewArgs._items	= arguments.collectionMaxRows>
			</cfif>
			<cfloop query="arguments.collection" startrow="#arguments.collectionStartRow#" endrow="#(arguments.collectionStartRow+viewArgs._items)-1#">
				<cfscript>
					// setup local cvariables
					viewArgs._counter  = arguments.collection.currentRow;
					viewArgs[ arguments.collectionAs ] = arguments.collection;
					// prepend the delim
    ...
    <cffunction name="renderViewComposite" output="false" access="public" returntype="any" hint="Render a view composite">
		...
		<cfargument name="viewHelperPath">
		<cfargument name="args"/>
		<cfargument name="_threadsafe" type="boolean" required="false" default="false" />
	...
	<cffunction name="renderLayout" access="Public" hint="Renders the current layout + view Combinations if declared." output="false" returntype="any">
		...
		<cfargument name="args"   		type="any" 	required="false" default="#getRequestContext().getCurrentViewArgs()#" hint="An optional set of arguments that will be available to this layouts/view rendering ONLY"/>
		...
		<cfif len(trim(arguments.view))>
			<cfset event.setValue( name="__layoutExplicitView", value=arguments.view, private=true ) />
		</cfif>
	...
	<cffunction name="locateLayout" output="false" access="public" returntype="any" hint="Locate the layout to render">
		...
		<cfscript>
			...
			if ( len( moduleName ) AND structKeyExists( instance.modulesConfig, moduleName ) ){
			...
			return "/#instance.appMapping#/#instance.layoutsConvention#/#arguments.layout#";
		</cfscript>
	</cffunction>
	...
	<cffunction name="locateModuleLayout" output="false" access="public" returntype="any" hint="Locate the view to render using module logic">
		...
		<cfscript>
			...
			if( structKeyExists(instance.modulesConfig,moduleName)){
	...
	<cffunction name="locateModuleView" output="false" access="public" returntype="any" hint="Locate the view to render using module logic">
		...
		<cfscript>
			...
			if( len(arguments.module) and arguments.explicitModule AND structKeyExists(instance.modulesConfig,arguments.module) ){
...
<!------------------------------------------- PRIVATE ------------------------------------------->
	...
	<cffunction name="_getViewDirectories" access="private" returntype="array" output="false">
		<cfscript>
	...
	<cffunction name="_getLayoutDirectories" access="private" returntype="array" output="false">
		<cfscript>
