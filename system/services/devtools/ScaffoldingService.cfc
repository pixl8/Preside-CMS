...
// PUBLIC API METHODS
	...
	public array function scaffoldCrudAdmin(
		...
		var templateVars = {
			...
			, objectNamePlural   = arguments.objectNamePlural
			, handlerRoot        = arguments.handler
			, handlerFolder      = arguments.handler.replace( ".", "/", "all" )
			, iconClass          = arguments.icon
			, pageIcon           = arguments.icon.reReplace( "^fa-", "" )
			, auditCategory      = arguments.auditCategory
		...
		var readAndReplaceTemplate = function( templateName ){
		...
		var templates = {
			...
			, indexcfm    = { content=readAndReplaceTemplate( "index.cfm.txt"            ), path="views/admin/#templateVars.handlerFolder#/index.cfm" }
			, editcfm     = { content=readAndReplaceTemplate( "edit.cfm.txt"             ), path="views/admin/#templateVars.handlerFolder#/edit.cfm" }
			, addcfm      = { content=readAndReplaceTemplate( "add.cfm.txt"              ), path="views/admin/#templateVars.handlerFolder#/add.cfm" }
			, historycfm  = { content=readAndReplaceTemplate( "versionHistory.cfm.txt"   ), path="views/admin/#templateVars.handlerFolder#/versionHistory.cfm" }
			, gridcfm     = { content=readAndReplaceTemplate( "_gridActions.cfm.txt"     ), path="views/admin/#templateVars.handlerFolder#/_gridActions.cfm" }
			, histactscfm = { content=readAndReplaceTemplate( "_historyActions.cfm.txt"  ), path="views/admin/#templateVars.handlerFolder#/_historyActions.cfm" }
			, auditcfm    = { content=readAndReplaceTemplate( "auditLogRenderer.cfm.txt" ), path="views/renderers/content/auditLogEntry/#templateVars.auditCategory#.cfm" }
			, props       = { content=readAndReplaceTemplate( "general.properties"       ), path="i18n/#templateVars.translationFile#.properties" }
		...
		for( var templateId in templates ) {
			...
			var filePath = rootPath & template.path;

			_ensureDirectoryExists( GetDirectoryFromPath( filePath ) );

			FileWrite( filePath, template.content );
			scaffoldedFiles.append( rootPath & template.path );
		}

		scaffoldedFiles.sort( "textnocase" );
