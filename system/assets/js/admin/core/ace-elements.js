...
presideJQuery(function() {
	//at some places we try to use 'tap' event instead of 'click' if presideJQuery mobile plugin is available
	window['ace'].click_event = presideJQuery.fn.tap ? "tap" : "click";
});
...
(function($ , undefined) {
	...
	Ace_File_Input.prototype.show_file_list = function($files) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			if((/\.(jpe?g|png|gif|svg|bmp|tiff?)$/i).test(filename)) {
				fileIcon = 'fa fa-picture';
			}
			else if((/\.(mpe?g|flv|mov|avi|swf|mp4|mkv|webm|wmv|3gp)$/i).test(filename)) fileIcon = 'fa fa-film';
			else if((/\.(mp3|ogg|wav|wma|amr|aac)$/i).test(filename)) fileIcon = 'fa fa-music';


			if(!this.well_style) this.$label.find('.file-name').attr({'data-title':filename}).find('[class*="fa-"]').attr('class', fileIcon);
			else {
				...
				var can_preview = hasFileReader && this.settings.thumbnail
						&&
						( (type.length > 0 && type.match('image')) || (type.length == 0 && fileIcon == 'fa fa-picture') )//the second one is for Android's default browser which gives an empty text for file.type
				if(can_preview) {
					var self = this;
	...
	Ace_File_Input.prototype.reset_input = function() {
	  this.$label.attr({'data-title':this.settings.btn_choose, 'class':'file-label'})
			...
			.find('[class*="fa-"]').attr('class', this.settings.no_icon)
			.prev('img').remove();
			if(!this.settings.no_icon) this.$label.find('[class*="fa-"]').remove();
...
(function($ , undefined) {
	$.fn.ace_spinner = function(options) {
		...
		this.each(function() {
			var icon_up = options.icon_up || 'fa fa-chevron-up'
			var icon_down = options.icon_down || 'fa fa-chevron-down'
	...
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		this.each(function() {
			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					case 'font':
						toolbar += ' <a class="btn btn-sm '+className+' dropdown-toggle" data-toggle="dropdown" title="'+button.title+'"><i class="'+button.icon+'"></i><i class="fa fa-angle-down fa fa-on-right"></i></a> ';
