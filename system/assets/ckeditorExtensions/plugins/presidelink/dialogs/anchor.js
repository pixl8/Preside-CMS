...
CKEDITOR.dialog.add( 'anchor', function( editor ) {
	...
	return {
		title: editor.lang.presidelink.anchor.title,
		minWidth: 300,
		minHeight: 60,
		onOk: function() {
		...
		onShow: function() {
			var sel = editor.getSelection(),
				...
				linkElement = fakeSelected ?
					CKEDITOR.plugins.presidelink.tryRestoreFakeAnchor( editor, fullySelected ) :
		...
		contents: [
			{
			id: 'info',
			label: editor.lang.presidelink.anchor.title,
			accessKey: 'I',
			elements: [
				{
				type: 'text',
				id: 'txtName',
				label: editor.lang.presidelink.anchor.name,
				required: true,
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.presidelink.anchor.errorName );
						return false;
					}
					return true;
				}
