...
( function( $ ) {
	CKEDITOR.plugins.add( 'presidelink', {
		...
		init: function( editor ) {
			...
			if ( editor.ui.addButton ) {
				...
				editor.ui.addButton( 'PresideAnchor', {
					label: editor.lang.presidelink.anchor.toolbar,
					command: 'anchor',
					toolbar: 'links,30'
				} );
			}
			...
			editor.on( 'doubleclick', function( evt ) {
				...
				if ( !element.isReadOnly() ) {
					...
					} else if ( CKEDITOR.plugins.presidelink.tryRestoreFakeAnchor( editor, element ) )
			...
			if ( editor.addMenuItems ) {
				editor.addMenuItems( {
					anchor: {
						label: editor.lang.presidelink.anchor.menu,
						command: 'anchor',
						group: 'anchor',
						order: 1
					},
					...
					removeAnchor: {
						label: editor.lang.presidelink.anchor.remove,
			...
			if ( editor.contextMenu ) {
				editor.contextMenu.addListener( function( element, selection ) {
					...
					var anchor = CKEDITOR.plugins.presidelink.tryRestoreFakeAnchor( editor, element );

					if ( !anchor && !( anchor = CKEDITOR.plugins.presidelink.getSelectedLink( editor ) ) )
						return null;
		...
		afterInit: function( editor ) {
			...
			if ( pathFilters ) {
				pathFilters.push( function( element, name ) {
					if ( name == 'a' ) {
						if ( CKEDITOR.plugins.presidelink.tryRestoreFakeAnchor( editor, element ) || ( element.getAttribute( 'name' ) && ( !element.getAttribute( 'href' ) || !element.getChildCount() ) ) )
							return 'anchor';
					}
				} );
			}
	...
	function protectEmailLinkAsFunction( editor, email ) {
		var plugin = editor.plugins.presidelink,
	 ...
	 * @singleton
	 */
	CKEDITOR.plugins.presidelink = {
		/**
		 * Get the surrounding link element of the current selection.
		 *
		 *		CKEDITOR.plugins.presidelink.getSelectedLink( editor );
		 *
	...
	CKEDITOR.removeAnchorCommand.prototype = {
		exec: function( editor ) {
			var sel = editor.getSelection(),
				...
				anchor;
			if ( sel && ( anchor = sel.getSelectedElement() ) && ( !anchor.getChildCount() ? CKEDITOR.plugins.presidelink.tryRestoreFakeAnchor( editor, anchor ) : anchor.is( 'a' ) ) )
				anchor.remove( 1 );
			else {
				if ( ( anchor = CKEDITOR.plugins.presidelink.getSelectedLink( editor ) ) ) {
