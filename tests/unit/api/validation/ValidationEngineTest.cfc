component output="false" extends="tests.resources.HelperObjects.PresideTestCase" {
	...
	function test01_newRuleset_shouldReturnTheRulesetArray(){
		...
		var expected = [
			  { fieldName="another_field"    , fieldLabel="", validator="email"    , params={ validDomains = ["gmail.com"] }, paramsFieldLabel={}, message=""                          , serverCondition="${yet_another_field} eq 'test'", clientCondition="${yet_another_field}.val() === 'test'" }
			, { fieldName="yet_another_field", fieldLabel="", validator="maxLength", params={ maxLength = 87 }              , paramsFieldLabel={}, message="{bundle:some.resource.key}", serverCondition="", clientCondition="" }
		];
