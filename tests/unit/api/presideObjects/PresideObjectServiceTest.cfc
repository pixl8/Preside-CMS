...
<!--- Filter Collision Prevention Tests --->
	<cffunction name="test089_selectData_shouldPreventCollisionBetweenDirectFilterAndExtraFilter" returntype="void">
		<cfscript>
			...
			result = poService.selectData(
				...
				, filter       = { "collision_test_object.status" = [ "inactive", "pending" ] }
				, extraFilters = [{
					filter = { "collision_test_object.status" = [ "active", "pending" ] }
				}]
			);

			super.assertEquals( 1, result.recordCount, "Expected exactly 1 record with status 'pending'" );
			super.assertEquals( "pending", result.status, "Expected the record to have status 'pending'" );
			super.assertEquals( "Test Object 2", result.label, "Expected correct record returned" );
		</cfscript>
	</cffunction>

	<cffunction name="test090_selectData_shouldPreventCollisionWithPriorityFilters" returntype="void">
		<cfscript>
			var poService  = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var testObjId1 = "";
			var testObjId2 = "";
			...
			result = poService.selectData(
				  objectName   = "collision_test_object"
				, filter       = { "collision_test_object.priority" = [ 1, 2, 3 ] }
				, extraFilters = [{
					filter = { "collision_test_object.priority" = [ 2, 3, 4 ] }
				}]
			);

			super.assertEquals( 1, result.recordCount, "Expected exactly 1 record with priority 2" );
			super.assertEquals( 2, result.priority, "Expected the record to have priority 2" );
			super.assertEquals( "Test Object 2", result.label, "Expected correct record returned" );
		</cfscript>
	</cffunction>

	<cffunction name="test091_selectData_shouldPreventCollisionBetweenMultipleExtraFilters" returntype="void">
		<cfscript>
			var poService  = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var testObjId1 = "";
			var testObjId2 = "";
			...
			testObjId4 = poService.insertData( objectName="collision_test_object", data={
				  label    = "Test Object 4"
				, status   = "archived"
				, priority = 4
			});
			...
			result = poService.selectData(
				  objectName   = "collision_test_object"
				, filter       = { "collision_test_object.status" = [ "active", "pending", "inactive", "archived" ] }
				, extraFilters = [
					{ filter = { "collision_test_object.status" = [ "active", "pending" ] } },
					{ filter = { "collision_test_object.status" = [ "pending", "inactive" ] } }
				]
			);
			...
			for ( var row in result ) {
				ArrayAppend( statuses, row.status );
			}

			super.assertEquals( "pending", statuses[1], "Expected 'pending' status in results" );
		</cfscript>
	</cffunction>

	<cffunction name="test092_selectData_shouldGenerateUniqueHashBasedParameterNames" returntype="void">
		<cfscript>
			var poService        = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var result           = "";
			var sql              = "";
			var params           = [];
			var hasDirectParam   = false;
			var hasHashedParam1  = false;
			var hasHashedParam2  = false;

			poService.dbSync();

			result = poService.selectData(
				  objectName          = "collision_test_object"
				, filter              = { "collision_test_object.status" = [ "active" ] }
				, extraFilters        = [
					{ filter = { "collision_test_object.status" = [ "pending" ] } },
					{ filter = { "collision_test_object.status" = [ "inactive" ] } }
				]
				, getSqlAndParamsOnly = true
			);
			...
			super.assert( FindNoCase( "collision_test_object.status", sql ), "Expected field reference in SQL" );

			for ( var param in params ) {
				if ( param.name == "collision_test_object__status" ) {
					hasDirectParam = true;
				} else if ( FindNoCase( "ef_", param.name ) && FindNoCase( "collision_test_object__status", param.name ) ) {
					if ( !hasHashedParam1 ) {
						hasHashedParam1 = true;
					} else {
						hasHashedParam2 = true;
					}
				}
			}

			super.assert( hasDirectParam, "Expected direct filter parameter" );
			super.assert( hasHashedParam1, "Expected first hashed parameter" );
			super.assert( hasHashedParam2, "Expected second hashed parameter" );
			super.assert( ArrayLen( params ) >= 3, "Expected at least 3 parameters (1 direct + 2 hashed)" );
		</cfscript>
	</cffunction>

	<cffunction name="test093_selectData_shouldHandleArrayValuesInCollidingFilters" returntype="void">
		<cfscript>
			var poService  = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var testObjId1 = "";
			var testObjId2 = "";
			...
			result = poService.selectData(
				  objectName   = "collision_test_object"
				, filter       = { "collision_test_object.status" = [ "active", "pending", "inactive" ] }
				, extraFilters = [{
					filter = { "collision_test_object.status" = [ "pending" ] }
				}]
			);

			super.assertEquals( 1, result.recordCount, "Expected exactly 1 record" );
	...
	<cffunction name="test094_selectData_shouldApplyAllFiltersAsLogicalAND" returntype="void">
		<cfscript>
			var poService  = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var testObjId1 = "";
			var testObjId2 = "";
			...
			super.assertEquals( 2, result.recordCount, "Expected exactly 2 records" );

			for ( var row in result ) {
				super.assert( ListFind( "pending,inactive", row.status ), "Should have 'pending' or 'inactive' status" );
				super.assert( ListFind( "2,3", row.priority ), "Should have priority 2 or 3" );
			}
		</cfscript>
	</cffunction>

	<cffunction name="test095_selectData_shouldNotAffectNonCollidingFilters" returntype="void">
		<cfscript>
			var poService  = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var testObjId1 = "";
			var testObjId2 = "";
			...
			result = poService.selectData(
				...
				, filter       = { "collision_test_object.status" = [ "active", "pending" ] }
				, extraFilters = [{
					filter = { "collision_test_object.priority" = [ 1 ] }
				}]
			);

			super.assertEquals( 1, result.recordCount, "Expected exactly 1 record" );
			super.assertEquals( "active", result.status, "Expected 'active' status" );
			super.assertEquals( 1, result.priority, "Expected priority 1" );
		</cfscript>
	</cffunction>

	<cffunction name="test096_selectData_shouldHandleComplexMultiFilterCollisionScenarios" returntype="void">
		<cfscript>
			var poService  = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var testObjId1 = "";
			var testObjId2 = "";
			...
			result = poService.selectData(
				...
				, extraFilters = [
					{ filter = { "collision_test_object.status" = [ "pending", "inactive" ] } },
					{ filter = { "collision_test_object.priority" = [ 2, 3 ] } },
					{ filter = { "collision_test_object.priority" = [ 1, 2 ] } }
				]
			);
	...
	<cffunction name="test099_selectData_shouldPreventCollisionWithCustomParameterNames" returntype="void">
		<cfscript>
			var poService  = _getService( objectDirectories=[ "/tests/resources/PresideObjectService/filterCollisionTesting/" ] );
			var testObjId1 = "";
			var testObjId2 = "";
			...
			result = poService.selectData(
				  objectName   = "collision_test_object"
				, filter       = "collision_test_object.status IN ( :allowedStatusesFilter )"
				, filterParams = {
					allowedStatusesFilter = { value = [ "active", "pending" ], type = "cf_sql_varchar", list = true }
				}
				, extraFilters = [{
					filter = { "collision_test_object.status" = [ "pending", "inactive" ] }
				}]
			...
			super.assertEquals( 1, result.recordCount, "Expected exactly 1 record with custom parameter collision prevention" );
			super.assertEquals( "pending", result.status, "Expected 'pending' status" );
			super.assertEquals( "Test Object 2", result.label, "Expected correct record" );
		</cfscript>
	</cffunction>
